...
//				$('.lxm_main').css('z-index','2')
				...
				$('.lxm_login_wrapper').css('transform','translateX(-13%)')
			})
...
//				$('.lxm_main').css('opacity','0')
				...
				$('.lxm_login_wrapper').css('transform','translateX(-50%)')
			})
						...
						$.ajax({
							...
							success:function(e){
								...
								if(e.result){
									...
									sessionStorage.teacherName = e.result[0].xingming
									sessionStorage.classes = e.result[0].class;
									sessionStorage.teacherTel = e.result[0].tel
								}
								if(e.flag ==1){
									setTimeout(function(){
